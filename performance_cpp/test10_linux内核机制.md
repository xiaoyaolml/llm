...
> NOTE：该 CFS 实现是教学级近似模型，目的是帮助理解 `vruntime` 与权重关系；真实内核调度还涉及睡眠补偿、负载跟踪、调度域平衡等复杂机制。
...
> NOTE：这里“零拷贝”是相对 `read()` 场景下避免“内核缓冲区 → 用户缓冲区”的额外复制；缺页处理、页表建立和缓存管理仍然会产生开销。
...
| fd 上限 | 1024 | 受进程 fd 上限约束 | 受进程 fd 上限约束 |
| 复杂度 | O(n) | O(n) | O(1) 就绪通知 |
| 拷贝 | 每次全量 | 每次全量 | 仅 ctl 时 |
| 触发模式 | LT | LT | LT + ET |

> NOTE：`epoll` 常说的 “O(1)” 主要指就绪事件通知路径避免了每次全量扫描；端到端开销仍受活跃连接数、回调分发和用户态处理影响。
...
> NOTE：`sendfile/splice` 收益依赖内核版本、文件系统、网卡 offload 和协议栈路径（例如 TLS）。某些场景会退化为含复制路径，应以实测结果为准。
...
> NOTE：参数不存在跨业务“通用最优值”。建议从基线压测出发逐项调优，并配合灰度发布与回滚策略。
